...
from backend.models.user import Employee, Apprentice, DepartmentManager, Executive, Admin
...
class TestPermissions(unittest.TestCase):
    ...
    def test_apprentice_has_employee_permissions(self):
        apprentice = Apprentice(7, "Tim Auszubildender", "1234", "Verkauf")

        self.assertEqual(apprentice.role, "apprentice")
        self.assertTrue(apprentice.can_clock_in())
        self.assertFalse(apprentice.can_create_reports())
        self.assertFalse(apprentice.can_manage_users())
        self.assertFalse(apprentice.can_view_all_times())
